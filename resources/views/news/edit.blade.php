...
<div class="dashboard-container">
    ...
    <main class="main-content">
        ...
        <div class="form-container">
            <h2><i class="fas fa-edit"></i> Edit News Article</h2>

            <form action="{{ route('news.update', $news) }}" method="POST" enctype="multipart/form-data" id="editForm">
                @csrf
                @method('PUT')

                <label for="title"><i class="fas fa-heading"></i> Title</label>
                <input type="text" id="title" name="title" value="{{ old('title', $news->title) }}" required placeholder="Enter news title">

                <label for="content"><i class="fas fa-align-left"></i> Content</label>
                <textarea id="content" name="content" required placeholder="Enter news content">{{ old('content', $news->content) }}</textarea>
                <small><i class="fas fa-info-circle"></i> Write the full news article content here</small>

                <label for="current-image"><i class="fas fa-image"></i> Current Image</label>
                <div class="current-image">
                    @if($news->image)
                        <img src="{{ asset('storage/' . $news->image) }}" alt="Current news image">
                    @else
                ...
                <small><i class="fas fa-info-circle"></i> Recommended: 800x400px, max 2MB (leave empty to keep current image)</small>

                <!-- Image Preview -->
                <div class="image-preview-container" id="imagePreview">
                    <img id="previewImage" class="image-preview" src="" alt="New Image Preview">
                </div>

                <!-- Form Actions -->
                <div class="form-actions">
                    <button type="submit" class="news-btn update">
